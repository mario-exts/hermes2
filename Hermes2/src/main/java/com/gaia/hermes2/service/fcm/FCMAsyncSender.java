...
public class FCMAsyncSender extends BaseLoggable implements Closeable {
	...
	public FCMAsyncSender(String secretKey) {
	...
	public void send(Message message, List<String> regIds, Callback<MulticastResult> successCallback,
		...
		PuObject params = FCMHelper.convertMessageToPuObject(message);
		params.set(JSON_REGISTRATION_IDS, regIds);

		RequestBuilder builder = RequestBuilder.post(Constants.FCM_SEND_ENDPOINT).addHeader("Content-Type", DEFAULT_CONTENT_TYPE)
			...
			public void apply(HttpResponse response) {
				if (response == null) {
					...
					if (!(puElement instanceof PuObject) || response.getStatusLine().getStatusCode() != 200) {
						Exception exception = new Exception("An error responsed by FCM --> " + puElement.toString());
						if (failureCallback != null) {
